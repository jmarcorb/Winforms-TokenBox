    ...
    public static class Icons
    {
        ...
        public class UnableToExtractIconsException : Exception
        {
            public UnableToExtractIconsException(string fileName, int firstIconIndex, int iconCount)
                : base(string.Format("Tried to extract {2} icons starting from the one with id {1} from the \"{0}\" file but failed", fileName, firstIconIndex, iconCount))
            {
            }
        }
                                                ...
                                                SystemIconSize size)
        {
            ...
            if (extension != null && extension != string.Empty)
            {
                ...
                ExtensionKey = Root.OpenSubKey(extension);
            }
            ...
            if (ExtensionKey == null)
            {
                ...
                ApplicationKey =  Root.OpenSubKey(ExtensionKey.GetValue("").ToString());
            }
            //gets the name of the file that have the icon.
            RegistryKey rk = ApplicationKey.OpenSubKey("DefaultIcon");
            if(rk==null)
            {
                ApplicationKey = Root.OpenSubKey("Unknown");
                rk = ApplicationKey.OpenSubKey("DefaultIcon");
            }
            string IconLocation = rk.GetValue("").ToString();
